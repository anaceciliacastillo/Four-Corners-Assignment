class Problem:
    ...
    def h(self, state):
        ...
        pacman_position, remaining_dots = state  # unpack the current state
        if not remaining_dots:
            return 0  

        # Step 1: Initialize heuristic cost and current position
        mst = {dot: False for dot in remaining_dots}  
        heuristic_cost = 0  
        current_position = pacman_position  

        # Step 2: Loop until all dots have been visited
        while not all(mst.values()):  
            min_distance = float('inf')
            closest_dot = None

            # Step 3: Find the closest unvisited dot
            for dot in remaining_dots:
                if not mst[dot]:  
                    distance = len(self.BFS(current_position, dot))  
                    if distance < min_distance:
                        min_distance = distance
                        closest_dot = dot

            # Step 4: Update heuristic cost and mark closest dot as visited
            heuristic_cost += min_distance
            mst[closest_dot] = True  
            current_position = closest_dot  
